...
func resolveEffectiveTLSConfig(ctx context.Context, restCfg *rest.Config, opts ctrlOpts) *sdktls.TLSConfig {
	...
	if err != nil {
		if errors.Is(err, errNoNamespace) {
			setupLog.Info("Not running in a cluster; skipping the ocm-tls-profile ConfigMap watch")
		} else {
			setupLog.Error(err, "Failed to get operator namespace; skipping the ocm-tls-profile ConfigMap watch")
		}
	...
	tlsCfg, err := resolveTLSConfig(ctx, opts.tlsMinVersion, opts.tlsCipherSuites, kubeClient, operatorNamespace,
		func() {
			setupLog.Info("TLS configuration changed; exiting so the pod restarts with the new settings")
			os.Exit(0)
		},
	)
	if err != nil {
		setupLog.Error(err, "Failed to resolve TLS configuration; falling back to defaults")
	...
	setupLog.Info("Effective TLS configuration",
...
// controller's own namespace. sdktls.StartTLSConfigMapWatcher lists/watches with a metadata.name
// field selector matching ConfigMapName, which is what lets list/watch be scoped by resourceNames
// here.
