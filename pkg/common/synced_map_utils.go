...
type SyncedPolicyMap struct {
	PolicyMap map[string]*policyv1alpha1.CertPolicy
	//Mx for making the map thread safe
	Mx sync.RWMutex
}

//GetObject used for fetching objects from the synced map
func (spm *SyncedPolicyMap) GetObject(key string) (value *policyv1alpha1.CertPolicy, found bool) {
...
func (spm *SyncedPolicyMap) AddObject(key string, plc *policyv1alpha1.CertPolicy) {

	spm.Mx.Lock()
	defer spm.Mx.Unlock()
	//check if the map is initialized, if not initilize it
	if spm.PolicyMap == nil {
		spm.PolicyMap = make(map[string]*policyv1alpha1.CertPolicy)
	}
